...
import { hostCoords, severityColor, severityTier, useZabbixHosts, useZabbixProblems, type ZHost } from "@/lib/zabbix";
import { Loader2, Maximize2, Minimize2, Activity, AlertTriangle, MapPin, Globe2 } from "lucide-react";
...
interface MapViewProps {
  /** When true, fills the parent container without rounded chrome. */
  embedded?: boolean;
  /** Initial map height (used when not in NOC wall fullscreen). */
  height?: string;
  /** Show NOC fullscreen toggle button. */
  showNocToggle?: boolean;
}

export const MapView = ({ embedded = false, height = "calc(100vh - 12rem)", showNocToggle = true }: MapViewProps) => {
  ...
  const wrapperStyle: React.CSSProperties = nocWall
    ? { position: "fixed", inset: 0, zIndex: 50, background: "#000" }
    : { height };

  return (
    <div className={cn("relative overflow-hidden border border-border", !nocWall && !embedded && "rounded-xl", embedded && !nocWall && "rounded-lg")} style={wrapperStyle}>
      {/* HUD */}
      <div className="absolute left-3 top-3 z-[1000] flex flex-col gap-2">
        <div className="rounded-lg border border-border bg-card/95 p-3 shadow-lg backdrop-blur">
          <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
            ...
            <Stat icon={Globe2} label="Total" value={stats.total} />
          </div>
            ...
            {Object.entries(regions).map(([r, v]) => {
              ...
              return (
                <div key={r} className="mb-1 last:mb-0">
                  <div className="flex justify-between text-[10px]"><span>{r}</span><span className="font-mono">{v}</span></div>
      ...
      {showNocToggle && (
        <button
          ...
          title={nocWall ? "Exit NOC Wall (Esc)" : "Enter NOC Wall"}
        >
          {nocWall ? <Minimize2 className="h-4 w-4" /> : <Maximize2 className="h-4 w-4" />}
        </button>
      )}
            ...
            <MarkerClusterGroup chunkedLoading iconCreateFunction={createClusterIcon} maxClusterRadius={50}>
              {filtered.map((p) => (
                <Marker
                  ...
                  // @ts-expect-error pass-through option for cluster severity color
